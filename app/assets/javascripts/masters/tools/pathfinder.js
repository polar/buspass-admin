...
function myRouteCallback(code, result) {
	...
	switch (code) {
		case Yours.status.routeFinished:
            ...
            writeToCopyBox(myFirstRoute);
			break;
		case Yours.status.starting:
			//alert('starting');
			//message_div.attr("src","images/ajax-loader.gif");
			message_div.html('Calculating route <img src="/assets/yours/images/ajax-loader.gif">');
			break;
...
function updateWaypointCallback(waypoint) {
	if (waypoint !== undefined) {
        ...
        var newPoint = point.transform(waypoint.route.map.projection, waypoint.route.map.displayProjection);
        var data = newPoint.x.toFixed(6) + ',' + newPoint.y.toFixed(6);
		$("#route_via li[waypointnr='" + waypoint.position + "'] .wp_text").val(waypoint.name);
        $("#route_via li[waypointnr='" + waypoint.position + "'] .wp_location").val(data);
	}
}
