  ...
  elRTE.prototype.ui.prototype.buttons.sofa_image = function(rte, name) {
    ...
    $(document).on('click', '#cms_dialog .uploaded_files .file_info', function(){
      ...
      if (ui_control) {
        ...
        ui_control.cms_text = $(this).data('value');
        ui_control.set();
    ...
    // BUSME: We changed this to drop the "{{cms:file:name:image}}" or {{cms:file:name:link}}
    // instead of the actual <img src="">. So much for the WSYWIG editor, but we want to
    // maintain referential consistency when the template is copied.
    this.set = function(){
      self.rte.history.add();
        if (self.cms_text)  {
            self.rte.selection.insertHtml(self.cms_text);
        } else {
