...
# We are going return two types, Routes and Active VehicleJourneys.
  def route_journeys
    @routes = Network.where(:municipality_id => @municipality.id).all
    rs      = []
  ...
  def getNetworkDefinitionJSON(network)
    ...
    data[:_geoJSONUrl] = route_master_municipality_webmap_path(@master, @municipality, :ref => network.id, :format => "json")
