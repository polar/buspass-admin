class Masters::Deployments::Networks::Routes::WebmapController < Masters::Deployments::Networks::NetworkBaseController
  ...
  def getJourneySpecAsRoute(journey)
   ...
   def getJourneyAsRouteDefinitionJSON(journey)
     ...
     data[:_name]= view_context.link_to "#{journey.route.display_name} #{(Time.parse("0:00") + journey.start_time.minutes).strftime("%H:%M %P")}",
         master_deployment_network_vehicle_journey_path(@master, @deployment, @network, journey)
     data[:_code]="#{journey.route.code}"
     data[:_version]="#{journey.route.version}"
     data[:_geoJSONUrl]= route_master_deployment_network_route_webmap_path(@master, @deployment, @network, journey.route, :ref => journey.id, :format => "json" )
