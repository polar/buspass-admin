...
#
class Masters::Deployments::Networks::VehicleJourneys::WebmapController < Masters::Deployments::Networks::NetworkBaseController
  layout "empty"
 ...
 def getDefinitionJSON(timing_link)
   ...
   data[:_type] = 'route'
   data[:_name]= view_context.link_to "#{timing_link.from.common_name} - #{timing_link.to.common_name}", master_deployment_network_vehicle_journey_journey_pattern_timing_link_path(@master, @deployment, @network, @vehicle_journey, timing_link)
   data[:_code]="#{timing_link.position+1}"
   data[:_version]="#{timing_link.created_at.to_i}"
   data[:_geoJSONUrl]= route_master_deployment_network_vehicle_journey_webmap_path(:ref => timing_link.id, :network_id => @network.id, :master_id => @master.id, :deployment_id => @deployment.id, :vehicle_journey_id => @vehicle_journey.id, :format => "json" )
